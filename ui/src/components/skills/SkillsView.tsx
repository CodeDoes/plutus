...
const DEFAULT_META = { icon: "📦", color: "text-gray-400", bg: "bg-gray-800/40", border: "border-gray-800/60" };
...
export default function SkillsView() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-br from-violet-600/10 via-purple-600/5 to-fuchsia-600/10 border border-violet-500/20 rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-100/50 text-sm mt-1 max-w-xl">
          ...
          <div className="flex gap-3">
            ...
            <div className="text-center">
              ...
              <div className="text-gray-500 text-[10px]">Custom</div>
            </div>
            <div className="text-center">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-surface-deep border border-gray-800/60 rounded-xl p-1">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
              activeTab === tab.id
                ...
                : "text-gray-500 hover:text-gray-400 hover:bg-gray-800/40 border border-transparent"
            }`}
          >
            <span>{tab.icon}</span>
            <span>{tab.label}</span>
            {tab.count !== undefined && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-800/40">{tab.count}</span>
            )}
          </button>
        ))}
      </div>

      {/* ── Tab Content Panel — consistent width & min-height ── */}
        ...
        {activeTab === "browse" && (
          <div className="w-full space-y-4">
            ...
            <div className="flex gap-3">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full bg-surface border border-gray-800/60 rounded-xl px-4 py-2.5 text-sm text-gray-100 placeholder-white/30 focus:outline-none focus:border-violet-500/50 transition-colors"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-400 text-xs"
                  >
                    ✕
                  </button>
                )}
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  selectedCat === "all"
                    ? "bg-violet-500/20 text-violet-400 border border-violet-500/30"
                    : "bg-gray-800/40 text-gray-500 border border-gray-800/60 hover:bg-white/10"
                }`}
              >
                All ({skills.length})
              </button>
            ...
            {loading ? (
              ...
              <div className="w-full bg-surface border border-gray-800/60 rounded-xl py-20">
                <div className="text-center">
                  <div className="text-4xl mb-3">🔍</div>
                  <h3 className="text-gray-300 font-semibold mb-1">No skills found</h3>
                  <p className="text-gray-500 text-sm">
                    {search ? "Try adjusting your search or filter." : "Skills will appear here once Plutus is connected."}
                  </p>
                </div>
              </div>
            ) : (
                      ...
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <h3 className="text-sm font-semibold text-gray-100">{skill.app}</h3>
                        ...
                        <p className="text-gray-100/50 text-xs leading-relaxed">{skill.description}</p>
                      </div>
                      ...
                      {isExpanded && (
                        <div className="border-t border-gray-800/60 p-4 bg-white/[0.02] space-y-3">
                          ...
                          {skill.required_params.length > 0 && (
                            <div>
                              <div className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Required Parameters</div>
                          ...
                          {skill.optional_params.length > 0 && (
                            <div>
                              <div className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Optional</div>
                              <div className="flex flex-wrap gap-1">
                                {skill.optional_params.map((p) => (
                                  <span key={p} className="text-xs bg-gray-800/40 text-gray-500 px-2 py-0.5 rounded font-mono">
                                    {p}
                                  </span>
                                ))}
                              </div>
                          ...
                          {/* Triggers */}
                          <div>
                            <div className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Triggers</div>
                            <div className="flex flex-wrap gap-1">
                              {skill.triggers.slice(0, 6).map((t) => (
                                <span key={t} className="text-[10px] bg-gray-800/40 text-gray-500 px-1.5 py-0.5 rounded">
                                  "{t}"
                                </span>
                              ))}
                              {skill.triggers.length > 6 && (
                                <span className="text-[10px] text-gray-600">+{skill.triggers.length - 6} more</span>
                              )}
                            </div>
                          </div>
          ...
          <div className="w-full space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-lg bg-violet-500/10 text-violet-400 border border-violet-500/20 hover:bg-violet-500/20 transition-colors flex-shrink-0 ml-4"
              >
                + Import Skill
              </button>
            </div>
            ...
            {savedSkills.length === 0 ? (
              <div className="w-full bg-surface border border-gray-800/60 rounded-xl py-20">
                <div className="text-center">
                  <div className="text-4xl mb-3">🌱</div>
                  <h3 className="text-gray-300 font-semibold mb-1">No custom skills yet</h3>
                  ...
                  <div className="flex gap-3 justify-center mt-5">
                    <button
                      onClick={() => setActiveTab("import")}
                      className="text-xs px-4 py-2 rounded-lg bg-violet-500/10 text-violet-400 border border-violet-500/20 hover:bg-violet-500/20 transition-colors"
                    >
                      📥 Import a Skill
                    </button>
                    <button
                      onClick={() => setActiveTab("community")}
                      className="text-xs px-4 py-2 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-colors"
                    >
                      🌍 Browse Community
                    </button>
                  </div>
              ...
              <div className="w-full space-y-2">
                {savedSkills.map((skill) => {
                  const meta = getMeta(skill.category);
                  return (
                    <div key={skill.name} className="w-full bg-surface border border-gray-800/60 rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-800/40 text-gray-500">
                            v{skill.version || 1}
                          </span>
                        ...
                        {skill.reason && (
                          <p className="text-gray-600 text-[10px] mt-0.5 italic">Created because: {skill.reason}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-gray-600 text-xs">{skill.steps_count || "?"} steps</span>
                        <button
        ...
        {activeTab === "import" && (
          <div className="w-full space-y-6">
            ...
            <div className="w-full bg-surface border border-gray-800/60 rounded-xl p-6">
              <h3 className="text-gray-100/80 font-semibold mb-1 flex items-center gap-2">
    ...
    "steps": [
      {
        ...
        "params": { "app_name": "{{param1}}" }
                ...
                className="w-full h-48 bg-surface-deep border border-gray-800/60 rounded-xl p-4 text-xs font-mono text-gray-300 placeholder-gray-600 focus:outline-none focus:border-violet-500/50 resize-none transition-colors"
              />
            ...
            <div className="w-full bg-surface border border-gray-800/60 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-2">
                  <div className="text-gray-100/50 font-semibold">Required Fields</div>
                ...
                <div className="space-y-2">
                  <div className="text-gray-100/50 font-semibold">Optional Fields</div>
                  <div className="space-y-1">
                    {[
        ...
        {activeTab === "community" && (
          <div className="w-full space-y-6">
            {/* Community Hero */}
            <div className="w-full bg-gradient-to-br from-blue-600/10 via-indigo-600/5 to-violet-600/10 border border-blue-500/20 rounded-2xl p-8 text-center">
              <div className="text-5xl mb-4">🌍</div>
              <h2 className="text-xl font-bold text-gray-100 mb-2">Plutus Skill Community</h2>
              <p className="text-gray-500 text-sm max-w-lg mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-surface border border-gray-800/60 rounded-xl p-5">
                ...
                <h3 className="text-gray-100/80 font-semibold text-sm mb-1">Export & Share</h3>
                <p className="text-gray-500 text-xs leading-relaxed">
                  Go to <strong className="text-gray-100/50">My Skills</strong>, click <strong className="text-violet-400">Export</strong> on any skill,
                  and share the JSON file on GitHub, Discord, or any community platform.
                </p>
              </div>
              <div className="bg-surface border border-gray-800/60 rounded-xl p-5">
                <div className="text-2xl mb-2">📥</div>
                <h3 className="text-gray-100/80 font-semibold text-sm mb-1">Import from Others</h3>
                <p className="text-gray-500 text-xs leading-relaxed">
                  Download a <code className="text-violet-400/60">.json</code> skill file from the community,
                  then use the <strong className="text-gray-100/50">Import</strong> tab to add it to your Plutus.
                </p>
              </div>
              <div className="bg-surface border border-gray-800/60 rounded-xl p-5">
                <div className="text-2xl mb-2">🤖</div>
                <h3 className="text-gray-100/80 font-semibold text-sm mb-1">AI-Created Skills</h3>
                <p className="text-gray-500 text-xs leading-relaxed">
                  When Plutus creates a skill for you, it appears in <strong className="text-gray-100/50">My Skills</strong>.
            ...
            {/* Community Links */}
            <div className="w-full bg-surface border border-gray-800/60 rounded-xl p-6">
              <h3 className="text-gray-100/80 font-semibold mb-4 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <a
                  ...
                  className="flex items-center gap-3 p-4 rounded-xl bg-white/[0.03] border border-gray-800/60 hover:border-white/20 transition-colors group"
                >
                  <span className="text-2xl">💬</span>
                  <div>
                    <div className="text-gray-300 text-sm font-semibold group-hover:text-gray-100 transition-colors">
                      GitHub Discussions
                    </div>
                    <div className="text-gray-500 text-xs">Share skills, ask questions, suggest features</div>
                  </div>
                </a>
                <a
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-xs">Browse and download community-created skills</div>
                  </div>
                </a>
                <a
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-xs">Can't find what you need? Request it from the community</div>
                  </div>
                </a>
                <a
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-xs">Learn how to contribute skills to the official repo</div>
                  </div>
                </a>
              </div>
            </div>
      ...
      {exportingSkill && exportedJson && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface border border-gray-800/60 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            <div className="p-6 border-b border-gray-800/60 flex items-center justify-between">
              <div>
                <h3 className="text-gray-100 font-semibold">Export Skill</h3>
